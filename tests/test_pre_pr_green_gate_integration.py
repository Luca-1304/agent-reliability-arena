...
class PrePRGreenGateIntegrationTests(unittest.TestCase):
    ...
    def test_default_registry_reuses_canonical_existing_commands(self) -> None:
        specs = {
            ...
            for spec in default_check_specs(
                python_executable="python",
                temp_root=Path("TEMP"),
            )
        }
        self.assertEqual(
            specs["compile-source"].commands,
            (("python", "-m", "compileall", "-q", "src", "tests", "scripts"),),
        )
        self.assertEqual(
            specs["source-tests"].commands,
            (
                (
                    "python",
                    "-m",
        ...
        self.assertEqual(
            specs["git-operations-policy"].commands,
            (
                (
                    "python",
        ...
        self.assertEqual(
            specs["history-boundary-local"].commands,
    ...
    def test_installed_command_smoke_uses_full_provider_free_fixture_surface(self) -> None:
        ...
        self.assertEqual(
            tuple(command[0] for command in smoke),
            (
                "arena-run",
                "arena-replay",
                "arena-export-web",
    ...
    def test_wheel_build_is_no_deps_no_isolation_and_outside_workspace(self) -> None:
        ...
        self.assertEqual(
            command[command.index("--wheel-dir") + 1],
            str(temp_root / "dist"),
        )
